...
document.addEventListener('DOMContentLoaded', function () {
  ...
  wireFileChip_('swrFileDTL', 'swrChipDTL', 'swrFileFO');
  wireFileChip_('swrFileFO', 'swrChipFO', 'swrFileDTL');
...
/**
 * Shows a confirmation chip under an upload box as soon as a video is
 * picked, and — while only one of the two is chosen — says which angle
 * is still missing. Without this the box looked identical before and
 * after choosing a file, so there was nothing telling the student the
 * first one had taken or that a second was needed.
 *
 * Oversized files are caught here as well as on submit. Finding out at
 * pick time beats filling in the whole form first and being rejected.
 */
function wireFileChip_(inputId, chipId, otherInputId) {
  ...
  input.addEventListener('change', function () {
    ...
    if (file.size > MAX_FILE_MB * 1024 * 1024) {
      chip.innerHTML = '<div class="swr-error show">That video is ' +
        Math.round(file.size / 1024 / 1024) + 'MB — please keep it under ' +
        MAX_FILE_MB + 'MB. A shorter clip, or 1080p instead of 4K, usually does it.</div>';
      input.value = '';
      return;
    }

    var otherChosen = other && other.files && other.files[0];
    var message = otherChosen
      ? '✓ ' + file.name
      : '✓ ' + file.name + ' — now add the other angle';

    chip.innerHTML = '<div class="swr-file-chip">' + message + '</div>';
  });
}
...
function handleSubmit_(paymentReference, submitBtn) {
  ...
  var goalField = document.getElementById('swrGoal');
  var studentGoal = (goalField && goalField.value.trim()) || '';
